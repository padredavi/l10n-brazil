...
class CrmLeadTest(TransactionCase):
    ...
    def test_conversion(self):
        ...
        self.crm_lead_company.write({'stage_id':self.env.ref('crm.stage_lead1').id})
        #Check if the lead stage is "Open".
        self.assertEqual(self.crm_lead_company.stage_id.sequence, 1, 'Lead stage did not Open')
        ...
        self.assertEqual(self.crm_lead_company.type, 'opportunity', 'Lead is not converted to opportunity!')
        self.assertEqual(self.crm_lead_company.partner_id.id, self.env.ref("base.res_partner_2").id, 'Partner mismatch!')
        self.assertEqual(self.crm_lead_company.stage_id.id, self.env.ref("crm.stage_lead1").id, 'Stage of opportunity is incorrect!')

    def test_create_partner(self):
        """ Create a Partner and check the if the fields were filled """
        self.partner_id = self.env['crm.lead']._create_lead_partner(self.crm_lead_company)

        self.obj_partner = self.env['res.partner'].browse(self.partner_id)

        self.assertTrue(self.obj_partner.name, 'The creation of the partner have problems.')
        self.assertTrue(self.obj_partner.legal_name, 'The field Razão Social not was filled.')
        self.assertTrue(self.obj_partner.cnpj_cpf, 'The field CNPJ not was filled.')
        self.assertTrue(self.obj_partner.inscr_est, 'The field Inscrição Estadual not was filled')
        self.assertTrue(self.obj_partner.inscr_mun, 'The field Inscrição Municipal not was filled')
...
#TODO fix up .case_mark_won()
    ...
    def test_conversion_contact(self):
        """Test to convert a Lead"""
        #Set lead to Open stage
        self.crm_lead_contact.write({'stage_id':self.env.ref('crm.stage_lead1').id})
        #Check if the lead stage is "Open".
        self.assertEqual(self.crm_lead_contact.stage_id.sequence, 1, 'Lead stage did not Open')
        #Convert lead into opportunity for exiting customer
        self.crm_lead_contact.convert_opportunity(self.env.ref("base.res_partner_2").id)

        #Check details of converted opportunity
        self.assertEqual(self.crm_lead_contact.type, 'opportunity', 'Lead is not converted to opportunity!')
        self.assertEqual(self.crm_lead_contact.partner_id.id, self.env.ref("base.res_partner_2").id, 'Partner mismatch!')
        self.assertEqual(self.crm_lead_contact.stage_id.id, self.env.ref("crm.stage_lead1").id, 'Stage of opportunity is incorrect!')

    def test_create_contact(self):
        """ Create a Contact and check the if the fields were filled """
        self.partner_id = self.env['crm.lead']._create_lead_partner(self.crm_lead_contact)
        self.obj_partner = self.env['res.partner'].browse(self.partner_id)

        self.assertTrue(self.obj_partner.name, 'The creation of the partner have problems.')

#TODO CPF and RG not found :
        #self.assertTrue(self.obj_partner.cnpj_cpf, 'The field CPF not was filled.')
        #self.assertTrue(self.obj_partner.inscr_est, 'The field RG not was filled')
...
#TODO fix up .case_mark_won()
    ...
    def test_create_partner_IE(self):
        ...
        """
        self.partner_id = self.env['crm.lead']._create_lead_partner(self.crm_lead_company_1)
